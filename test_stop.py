"""First live write test: send STOP once while the desk is idle.

Safety: only run this while watching the desk with the physical controller
within reach. STOP should be a no-op when the desk isn't moving.
"""
...
async def main():
    ...
    async with BleakClient(device) as client:
        ...
        print()
        print("About to send STOP:", protocol.STOP_FRAME.hex(" ").upper())
        input("Press Enter to confirm you are watching the desk and send STOP... ")

        await client.write_gatt_char(WRITE_UUID, protocol.STOP_FRAME, response=True)
        print("STOP sent. Watching for any notifications for 5 seconds...")
